import torch

class GraphModule(torch.nn.Module):
    
    
    
    def forward(self, in_0 : torch.Tensor, in_1, in_2):
        tmp_1 = in_2 + in_1;  in_2 = in_1 = None
        tmp_2 = tmp_1.to(torch.float32);  tmp_1 = None
        tmp_3 = tmp_2.pow(2)
        tmp_4 = tmp_3.mean(-1, keepdim = True);  tmp_3 = None
        tmp_5 = tmp_4 + 1e-06;  tmp_4 = None
        tmp_6 = torch.rsqrt(tmp_5);  tmp_5 = None
        tmp_7 = tmp_2 * tmp_6;  tmp_2 = tmp_6 = None
        tmp_8 = tmp_7.to(torch.bfloat16);  tmp_7 = None
        tmp_9 = in_0 * tmp_8;  in_0 = tmp_8 = None
        tmp_10 = tmp_9[(slice(None, None, None), slice(0, None, None), slice(None, None, None))];  tmp_9 = None
        return (tmp_10,)
        